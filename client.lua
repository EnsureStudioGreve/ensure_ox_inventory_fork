...
local function canOpenInventory()
	...
	if PlayerData.dead or IsPedFatallyInjured(playerPed) or LocalPlayer.state.isDead then
        return shared.info('cannot open inventory', '(downed or fatally injured)')
	end
...
local defaultInventory = {
	type = 'newdrop',
	slots = shared.dropslots, --slots = shared.playerslots,
	weight = 0,
	maxWeight = shared.dropweight,--maxWeight = shared.playerweight,
	items = {}
}
...
function client.openInventory(inv, data)
    ...
    SendNUIMessage({
		...
		dict = "anim@heists@heist_corona@single_team",Add commentMore actions
...
RegisterNetEvent('ox_inventory:forceOpenInventory', function(left, right)
	...
	VirtualPed.startClonedPedPreview(cache.ped, {Add commentMore actions
